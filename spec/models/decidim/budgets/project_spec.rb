...
module Decidim::Budgets
  describe Project do
    ...
    describe "#paper_ballots" do
      let(:project) { create :project }
      let!(:paper_ballot_result) { create :paper_ballot_result, project: project, votes: 10 }

      it "returns the number of paper votes for a specific project" do
        expect(project.paper_ballots).to eq(10)
      end
    end
    ...
    describe "#total_votes" do
      ...
      it "returns the number of times a specific project has been selected through both online and paper votes" do
        expect(project.total_votes).to eq(11)
      end
    end
  end
end
